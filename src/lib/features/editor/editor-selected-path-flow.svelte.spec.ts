...
import Harness from './test-fixtures/EditorSelectedPathFlowHarness.svelte';
...
describe('selectedPath browser flow', () => {
	...
	it('updates controls on file switch while preview keeps following the selected file', async () => {
		render(Harness);
		...
		await userEvent.click(page.getByRole('button', { name: 'Hide controls' }));
		await expect.element(page.getByRole('button', { name: 'Show controls' })).toBeInTheDocument();
		await vi.waitFor(() => {
		...
		await userEvent.click(page.getByRole('button', { name: 'Show controls' }));
		await expect.element(page.getByText('No controls')).toBeInTheDocument();
		await expect.element(
			page.getByText('plain.ts has no template control metadata.')
		).toBeInTheDocument();
