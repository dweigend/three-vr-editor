...
describe('toViewerError', () => {
	it('normalizes Error instances', () => {
		...
		error.stack =
			'Error: Cube exploded\n    at renderFrame (/src/lib/features/editor/create-three-runtime.ts:88:13)\n';

		expect(toViewerError(error)).toEqual({
			title: 'Three.js viewer error',
			message: 'Cube exploded',
			location: '/src/lib/features/editor/create-three-runtime.ts:88:13',
			source: {
				column: 13,
				filePath: '/src/lib/features/editor/create-three-runtime.ts',
				line: 88
			},
			stack:
				'Error: Cube exploded\n    at renderFrame (/src/lib/features/editor/create-three-runtime.ts:88:13)\n'
...
describe('extractErrorLocation', () => {
	it('extracts a file location from a stack trace', () => {
		expect(
			extractErrorLocation(
				'Error: boom\n    at renderFrame (http://localhost:5173/src/lib/features/editor/create-three-runtime.ts:112:7)\n'
			)
		).toEqual({
			location: 'http://localhost:5173/src/lib/features/editor/create-three-runtime.ts:112:7',
			source: {
				column: 7,
				filePath: 'http://localhost:5173/src/lib/features/editor/create-three-runtime.ts',
				line: 112
			}
		});
	});
